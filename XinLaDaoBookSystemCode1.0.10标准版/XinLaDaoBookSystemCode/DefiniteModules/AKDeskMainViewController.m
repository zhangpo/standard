...
- (void)segmentClick:(UISegmentedControl*)sender
{
    ...
    NSString *padStr = [NSString stringWithFormat:@"%@%@",[AKFilePath getPadID],[[AKFilePath getDicCurrentPageConfig] objectForKey:@"number"]];
    NSMutableDictionary *parameterDic = [NSMutableDictionary dictionaryWithObjectsAndKeys:[[Singleton sharedSingleton].userInfo objectForKey:@"user"],@"userCode",@"",@"floor",@"",@"area",@"",@"state",padStr,@"deviceId",@"",@"tableNum", nil];
    _tabledict=parameterDic;
    CVLocalizationSetting *langSetting = [CVLocalizationSetting sharedInstance];
    ...
    {
    if ([[[NSUserDefaults standardUserDefaults]objectForKey:@"desk"]isEqualToString:[langSetting localizedString:@"Area"]]) {
        [parameterDic setValue:[[array objectAtIndex:segment.selectedSegmentIndex-1] objectForKey:@"AREARID"] forKey:@"area"];
        //        [parameterDic setValue:[NSString stringWithFormat:@"%d",segment.selectedSegmentIndex+1] forKey:@"area"];
        self.checkTableInfo = [NSDictionary dictionaryWithObjectsAndKeys:DESStr,@"area", nil];
        //        [parameterDic setValue:[NSString stringWithFormat:@"%d",segment.selectedSegmentIndex+1] forKey:@"area"];
        self.checkTableInfo = [NSDictionary dictionaryWithObjectsAndKeys:DESStr,@"area", nil];
    }else
        if ([[[NSUserDefaults standardUserDefaults]objectForKey:@"desk"]isEqualToString:[langSetting localizedString:@"Floor"]]){
            [parameterDic setObject:DESStr forKey:@"Floor"];
            self.checkTableInfo = [NSDictionary dictionaryWithObjectsAndKeys:DESStr,@"floor", nil];
        }else if ([[[NSUserDefaults standardUserDefaults]objectForKey:@"desk"]isEqualToString:[langSetting localizedString:@"Status"]]){
            /*
             '空闲'=A
             '开台点菜'=B
            ...
            for(int i=0;i<9;i++)
            {
                if (i<5) {
                    if (i==segment.selectedSegmentIndex) {
                        [parameterDic setObject:[NSString stringWithFormat:@"%d",i] forKey:@"state"];
                        self.checkTableInfo = [NSDictionary dictionaryWithObjectsAndKeys:[NSString stringWithFormat:@"%d",i],@"state", nil];
                    }
                }
                else
                {
                    if (i==segment.selectedSegmentIndex) {
                        [parameterDic setObject:[NSString stringWithFormat:@"%d",i+1] forKey:@"state"];
                        self.checkTableInfo = [NSDictionary dictionaryWithObjectsAndKeys:[NSString stringWithFormat:@"%d",i+1],@"state", nil];
                    }
                }
